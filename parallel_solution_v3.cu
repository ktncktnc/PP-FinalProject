...
ParallelSolutionV3::extractSeam(const int32_t *energyMap, uint32_t inputWidth, uint32_t inputHeight, uint32_t *seam) {
    ...
    u_int32_t middleRow = inputHeight / 2;
    ...
    // Trace back
    seam[inputHeight / 2] = minValCol1;
    seam[inputHeight / 2 + 1] = minValCol2;

    for (int r = int(inputHeight / 2 - 1); r >= 0; --r) {
        auto c = minValCol1;
        if (c > 0) {
    ...
    for (int r = int(inputHeight / 2 + 2); r < inputHeight; ++r) {
        auto c = minValCol2;
        if (c > 0) {
            if (energyMap[KernelFunction::convertIndex(r, c - 1, inputWidth)] <=
